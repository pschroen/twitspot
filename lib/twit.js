        ...
        stream.on('tweet', function (tweet) {
            ...
            if (match) {
                ...
                callbacks[callbackid] = function (data) {
                    if (data) {
                        var track = {name:data.name, artists:[{name:data.artists[0].name}], album:{images:[{url:'https://i.scdn.co/image/'+data.album.cover.split(':')[2]}]}, link:data.link};
...
function play(config) {
    if (tracks[0].track) {
        ...
        spot.send({message:'get', data:{link:tracks[0].track.link}, id:callbackid});
        callbackid++;
        nowplaying = tracks[0];
        twitspot(config);
    } else {
