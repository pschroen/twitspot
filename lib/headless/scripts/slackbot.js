...
function init(probe, load, callback) {
    ...
    rtm.on(RTM_EVENTS.MESSAGE, function (message) {
        ...
        var channelgroup = rtm.dataStore.getChannelGroupOrDMById(message.channel),
            user = rtm.dataStore.getUserById(message.user),
            tweet = {
                user: {
                    name: channelgroup.name ? "#"+channelgroup.name : "DM",
                    screen_name: user.name,
                    profile_image_url: user.profile.image_48
                },
                text: message.text
            };
...
function slack(probe, message, channel) {
    ...
    var channelgroup = rtm.dataStore.getChannelGroupOrDMById(channel);
    log(probe, channelgroup.name ? "Sending to #"+channelgroup.name : "Sending to "+rtm.dataStore.getUserById(channelgroup.user).name);
